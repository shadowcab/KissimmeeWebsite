...
(function ($) {
  ...
  $(document).on("cycle-bootstrap", function (e, opts, API) {
    ...
    function _calcNextSlide(start, count) {
      if (opts.allowWrap) return _modulo(start + count, opts.slideCount);
      else
        return _between(
          start + count,
          0,
  ...
  $.fn.cycle.transitions.carousel = {
    ...
    postInit: function (opts) {
      ...
      if (opts.carouselVisible)
        opts.carouselHops = Math.min(
          opts.carouselHops || 1,
          opts.slideCount - opts.carouselVisible + 1
        );
      else
        opts.carouselHops = Math.min(
          opts.carouselHops || 1,
    ...
    transition: function (opts, curr, next, fwd, callback) {
      ...
      if (opts.allowWrap === false) {
        ...
        if (fwd && opts.nextSlide < opts.currSlide) {
          ...
          moveBy = this.getScroll(
            opts,
            vert,
            opts.currSlide,
            opts.slideCount - opts.currSlide
          );
          ...
          moveBy = this.getScroll(
            ...
            vert,
            opts.nextSlide,
            opts.slideCount - opts.nextSlide
          );
